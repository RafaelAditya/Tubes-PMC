...
void loop() {
  ...
  snprintf(plaintext, MAX_MSG_LEN, "altitude_%d;speed_%d;orientation_%d", messageCounter, messageCounter, messageCounter);

  // unsigned long startTime = micros(); // Waktu awal

  // Publish plaintext message to "kelompok11" topic
  if (client.publish("kelompok11", plaintext)) {
